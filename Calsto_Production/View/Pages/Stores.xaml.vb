...
Public Class Stores
    ...
    Private StoresJobList As ObservableCollection(Of StoresModel)
    Private LotList As ObservableCollection(Of StoresLotModel)
    Private ProjectList As ObservableCollection(Of StoresProjModel)
    ...
    Private Sub LoadProjects()
        Try
            Dim data = StoresDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of StoresProjModel)(data)
    ...
    ' Load all Stores jobs
    Private Sub LoadStoresJobs(proNo As String)
        Try
            Dim data = StoresDBHelper.GetStoresJobs(proNo)
            StoresJobList = New ObservableCollection(Of StoresModel)(data)
            FormDG.ItemsSource = StoresJobList
        Catch ex As Exception
            MessageBox.Show("Error loading project list: " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub LoadLotIDs(jobc As String)
        Try
            Dim data = StoresDBHelper.GetStoresLots(jobc)
            LotList = New ObservableCollection(Of StoresLotModel)(data)
    ...
    Private Sub FormProj_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedProj As StoresProjModel = TryCast(ProjDG.SelectedItem, StoresProjModel)
        If selectedProj IsNot Nothing Then
            LoadStoresJobs(selectedProj.PROJECTNO)
    ...
    Private Sub dgLOT_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedlot As StoresLotModel = TryCast(JobcardDG.SelectedItem, StoresLotModel)
    ...
    Private Sub Change_lot_click(sender As Object, e As RoutedEventArgs)
        ...
        StoresDBHelper.EditLotEntry(transid, Lotid, newqty, createdBy, Remarks)

        Dim selectedJC As StoresModel = TryCast(FormDG.SelectedItem, StoresModel)
    ...
    Private Sub LotEntry_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim selectedJC As StoresModel = CType(FormDG.SelectedItem, StoresModel)
        ...
        ' Save to DB
        StoresDBHelper.CreateJobEntry(selectedJC.JC_no, JCQty, createdBy, Remarks)
        RefreshBOM()
        Refreshproj()
        Lot_qty.Text = ""
    ...
    Private Sub RefreshBOM()
        Try
            ...
            If FormDG.SelectedItem IsNot Nothing Then
                selectedWID = CType(FormDG.SelectedItem, StoresModel).WID
            End If

            ' 3. Get current project number
            Dim selectedProj = CType(ProjDG.SelectedItem, StoresProjModel)
            ...
            Dim rowToSelect = StoresJobList.FirstOrDefault(Function(x) x.WID = selectedWID)
    ...
    Private Sub Refreshproj()
        Try
            ...
            If ProjDG.SelectedItem IsNot Nothing Then
                selectedProjNo = CType(ProjDG.SelectedItem, StoresProjModel).PROJECTNO
            End If

            Dim projData = StoresDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of StoresProjModel)(projData)
            ProjDG.ItemsSource = ProjectList
