...
Public Class Inventory
    ...
    Private InventoryJobList As ObservableCollection(Of InventoryModel)
    Private LotList As ObservableCollection(Of InventoryLotModel)
    Private ProjectList As ObservableCollection(Of InventoryProjModel)
    ...
    Private Sub LoadProjects()
        Try
            Dim data = InventoryDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of InventoryProjModel)(data)
    ...
    ' Load all Inventory jobs
    Private Sub LoadInventoryJobs(proNo As String)
        Try
            Dim data = InventoryDBHelper.GetInventoryJobs(proNo)
            InventoryJobList = New ObservableCollection(Of InventoryModel)(data)
            FormDG.ItemsSource = InventoryJobList
        Catch ex As Exception
            MessageBox.Show("Error loading project list: " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub LoadLotIDs(jobc As String)
        Try
            Dim data = InventoryDBHelper.GetInventoryLots(jobc)
            LotList = New ObservableCollection(Of InventoryLotModel)(data)
    ...
    Private Sub FormProj_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedProj As InventoryProjModel = TryCast(ProjDG.SelectedItem, InventoryProjModel)
        If selectedProj IsNot Nothing Then
            LoadInventoryJobs(selectedProj.PROJECTNO)
    ...
    Private Sub dgLOT_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedlot As InventoryLotModel = TryCast(JobcardDG.SelectedItem, InventoryLotModel)
    ...
    Private Sub Change_lot_click(sender As Object, e As RoutedEventArgs)
        ...
        InventoryDBHelper.EditLotEntry(transid, Lotid, newqty, createdBy, Remarks)

        Dim selectedJC As InventoryModel = TryCast(FormDG.SelectedItem, InventoryModel)
    ...
    Private Sub LotEntry_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim selectedJC As InventoryModel = CType(FormDG.SelectedItem, InventoryModel)
        ...
        ' Save to DB
        InventoryDBHelper.CreateJobEntry(selectedJC.JC_no, JCQty, createdBy, Remarks)
        RefreshBOM()
        Refreshproj()
        Lot_qty.Text = ""
    ...
    Private Sub RefreshBOM()
        Try
            ...
            If FormDG.SelectedItem IsNot Nothing Then
                selectedWID = CType(FormDG.SelectedItem, InventoryModel).WID
            End If

            ' 3. Get current project number
            Dim selectedProj = CType(ProjDG.SelectedItem, InventoryProjModel)
            ...
            Dim rowToSelect = InventoryJobList.FirstOrDefault(Function(x) x.WID = selectedWID)
    ...
    Private Sub Refreshproj()
        Try
            ...
            If ProjDG.SelectedItem IsNot Nothing Then
                selectedProjNo = CType(ProjDG.SelectedItem, InventoryProjModel).PROJECTNO
            End If

            Dim projData = InventoryDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of InventoryProjModel)(projData)
            ProjDG.ItemsSource = ProjectList
