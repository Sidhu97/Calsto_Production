...
Public Class Laser
    ...
    Private LaserJobList As ObservableCollection(Of LaserModel)
    Private LotList As ObservableCollection(Of LaserLotModel)
    Private ProjectList As ObservableCollection(Of LaserProjModel)
    ...
    Private Sub LoadProjects()
        Try
            Dim data = LaserDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of LaserProjModel)(data)
    ...
    ' Load all Laser jobs
    Private Sub LoadLaserJobs(proNo As String)
        Try
            Dim data = LaserDBHelper.GetLaserJobs(proNo)
            LaserJobList = New ObservableCollection(Of LaserModel)(data)
            FormDG.ItemsSource = LaserJobList
        Catch ex As Exception
            MessageBox.Show("Error loading project list: " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub LoadLotIDs(jobc As String)
        Try
            Dim data = LaserDBHelper.GetLaserLots(jobc)
            LotList = New ObservableCollection(Of LaserLotModel)(data)
    ...
    Private Sub FormProj_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedProj As LaserProjModel = TryCast(ProjDG.SelectedItem, LaserProjModel)
        If selectedProj IsNot Nothing Then
            LoadLaserJobs(selectedProj.PROJECTNO)
    ...
    Private Sub dgLOT_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedlot As LaserLotModel = TryCast(JobcardDG.SelectedItem, LaserLotModel)
    ...
    Private Sub Change_lot_click(sender As Object, e As RoutedEventArgs)
        ...
        LaserDBHelper.EditLotEntry(transid, Lotid, newqty, createdBy, Remarks)

        Dim selectedJC As LaserModel = TryCast(FormDG.SelectedItem, LaserModel)
    ...
    Private Sub LotEntry_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim selectedJC As LaserModel = CType(FormDG.SelectedItem, LaserModel)
        ...
        ' Save to DB
        LaserDBHelper.CreateJobEntry(selectedJC.JC_no, JCQty, createdBy, Remarks)
        RefreshBOM()
        Refreshproj()
        Lot_qty.Text = ""
    ...
    Private Sub RefreshBOM()
        Try
            ...
            If FormDG.SelectedItem IsNot Nothing Then
                selectedWID = CType(FormDG.SelectedItem, LaserModel).WID
            End If

            ' 3. Get current project number
            Dim selectedProj = CType(ProjDG.SelectedItem, LaserProjModel)
            ...
            Dim rowToSelect = LaserJobList.FirstOrDefault(Function(x) x.WID = selectedWID)
    ...
    Private Sub Refreshproj()
        Try
            ...
            If ProjDG.SelectedItem IsNot Nothing Then
                selectedProjNo = CType(ProjDG.SelectedItem, LaserProjModel).PROJECTNO
            End If

            Dim projData = LaserDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of LaserProjModel)(projData)
            ProjDG.ItemsSource = ProjectList
