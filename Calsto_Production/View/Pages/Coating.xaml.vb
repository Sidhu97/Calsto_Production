...
Public Class Coating
    ...
    Private CoatingJobList As ObservableCollection(Of CoatingModel)
    Private LotList As ObservableCollection(Of CoatingLotModel)
    Private ProjectList As ObservableCollection(Of CoatingProjModel)
    ...
    Private Sub LoadProjects()
        Try
            Dim data = CoatingDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of CoatingProjModel)(data)
    ...
    ' Load all Coating jobs
    Private Sub LoadCoatingJobs(proNo As String)
        Try
            Dim data = CoatingDBHelper.GetCoatingJobs(proNo)
            CoatingJobList = New ObservableCollection(Of CoatingModel)(data)
            FormDG.ItemsSource = CoatingJobList
        Catch ex As Exception
            MessageBox.Show("Error loading project list: " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub LoadLotIDs(jobc As String)
        Try
            Dim data = CoatingDBHelper.GetCoatingLots(jobc)
            LotList = New ObservableCollection(Of CoatingLotModel)(data)
    ...
    Private Sub FormProj_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedProj As CoatingProjModel = TryCast(ProjDG.SelectedItem, CoatingProjModel)
        If selectedProj IsNot Nothing Then
            LoadCoatingJobs(selectedProj.PROJECTNO)
    ...
    Private Sub dgLOT_Sel_Changed(sender As Object, e As SelectionChangedEventArgs)
        Dim selectedlot As CoatingLotModel = TryCast(JobcardDG.SelectedItem, CoatingLotModel)
    ...
    Private Sub Change_lot_click(sender As Object, e As RoutedEventArgs)
        ...
        CoatingDBHelper.EditLotEntry(transid, Lotid, newqty, createdBy, Remarks)

        Dim selectedJC As CoatingModel = TryCast(FormDG.SelectedItem, CoatingModel)
    ...
    Private Sub LotEntry_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim selectedJC As CoatingModel = CType(FormDG.SelectedItem, CoatingModel)
        ...
        ' Save to DB
        CoatingDBHelper.CreateJobEntry(selectedJC.JC_no, JCQty, createdBy, Remarks)
        RefreshBOM()
        Refreshproj()
        Lot_qty.Text = ""
    ...
    Private Sub RefreshBOM()
        Try
            ...
            If FormDG.SelectedItem IsNot Nothing Then
                selectedWID = CType(FormDG.SelectedItem, CoatingModel).WID
            End If

            ' 3. Get current project number
            Dim selectedProj = CType(ProjDG.SelectedItem, CoatingProjModel)
            ...
            Dim rowToSelect = CoatingJobList.FirstOrDefault(Function(x) x.WID = selectedWID)
    ...
    Private Sub Refreshproj()
        Try
            ...
            If ProjDG.SelectedItem IsNot Nothing Then
                selectedProjNo = CType(ProjDG.SelectedItem, CoatingProjModel).PROJECTNO
            End If

            Dim projData = CoatingDBHelper.GetProjectNO()
            ProjectList = New ObservableCollection(Of CoatingProjModel)(projData)
            ProjDG.ItemsSource = ProjectList
