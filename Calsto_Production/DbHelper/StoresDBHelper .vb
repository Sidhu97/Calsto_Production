...
#Region "Project List"
    Public Shared Function GetProjectNO() As List(Of StoresProjModel)
        Dim Projectlist As New List(Of StoresProjModel)
        Using con As New SqlConnection(conString)
            con.Open()
            Using cmd As New SqlCommand("SELECT * FROM V_PROJ_Stores", con)
                Using rdr = cmd.ExecuteReader()
                    While rdr.Read()
                        Projectlist.Add(New StoresProjModel With {
...
#Region "Get Stores Jobs"
    Public Shared Function GetStoresJobs(proNo As String) As List(Of StoresModel)
        Dim StoresJobs As New List(Of StoresModel)

        Using con As New SqlConnection(conString)
            ' Add WHERE clause to use the parameter
            Dim query As String = "SELECT * FROM V_JOB_Stores WHERE [BOM No.] = @BOMNo"
            ...
            Try
                ...
                While reader.Read()
                    Dim job As New StoresModel With {
            ...
            Catch ex As Exception
                MessageBox.Show("Error loading Stores jobs: " & ex.Message)
            End Try
        End Using
...
#Region "Get Stores LOT IDs"
    Public Shared Function GetStoresLots(Jobno As String) As List(Of StoresLotModel)
        Dim Storeslots As New List(Of StoresLotModel)
        ...
        Using con As New SqlConnection(conString)
            ...
            Try
                ...
                Dim reader As SqlDataReader = cmd.ExecuteReader()
                While reader.Read()
                    Dim job As New StoresLotModel With {
